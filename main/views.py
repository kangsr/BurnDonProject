...
def page5(req):
    ...
    deposit = round((salary/12)-before) #연간저축액
    lastYear = round(gold/(after*12))
    goldAge = age + round(gold/(deposit*12)) #달성가능 나이
...
def page6(req):
    ...
    salary = data.salary # 연봉
    before = data.beforeExpense # 은퇴전생활비
    current = data.currentMoney # 현재저축액
    age = data.age

    monthDeposit = round((salary/12)-before) # 월저축액
    ...
    for i in range(10):
        ...
        if i==0:
            yearDeposit_arr.append(current)
        else:
